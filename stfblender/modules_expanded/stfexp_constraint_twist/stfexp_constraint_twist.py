...
_blender_property_name = "stf_exp_constraint_twist"
...
def _resolve_property_path_to_stf_func(context: STF_ExportContext, application_object: any, application_object_property_index: int, data_path: str) -> tuple[list[str], Callable[[int, any], any], list[int]]:
	if(match := re.search(r"^stf_exp_constraint_twist\[(?P<component_index>[\d]+)\].weight", data_path)):
		component = application_object.stf_exp_constraint_twist[int(match.groupdict()["component_index"])]
...
def _resolve_stf_property_to_blender_func(context: STF_ImportContext, stf_path: list[str], application_object: any) -> tuple[any, int, any, any, list[int], Callable[[int, any], any]]:
	...
	match(stf_path[1]):
		case "weight":
			for component_index, component in enumerate(application_object.stf_exp_constraint_twist):
				if(component.stf_id == blender_object.stf_id):
					break
			return None, 0, "OBJECT", "stf_exp_constraint_twist[" + str(component_index) + "].weight", 0, None
	return None
...
class STF_Module_STFEXP_Constraint_Twist(STF_BlenderComponentModule):
	...
	understood_application_property_path_parts = ["stf_exp_constraint_twist"]
...
def register():
	bpy.types.Object.stf_exp_constraint_twist = bpy.props.CollectionProperty(type=STFEXP_Constraint_Twist) # type: ignore
	bpy.types.Bone.stf_exp_constraint_twist = bpy.props.CollectionProperty(type=STFEXP_Constraint_Twist) # type: ignore

def unregister():
	if hasattr(bpy.types.Object, "stf_exp_constraint_twist"):
		del bpy.types.Object.stf_exp_constraint_twist
	if hasattr(bpy.types.Bone, "stf_exp_constraint_twist"):
		del bpy.types.Bone.stf_exp_constraint_twist
