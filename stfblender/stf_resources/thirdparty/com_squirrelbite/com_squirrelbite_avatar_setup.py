...
from .....stfblender_common import STF_ExportContext, STF_ImportContext, STFReportSeverity, STFReport, STF_Category, STF_TaskSteps, STF_ComponentResourceBase, STF_Handler_Component, STF_Component_Ref, add_component, export_component_base, import_component_base
...
def _draw_func_grab_toggle(layout: bpy.types.UILayout, element: Any) -> bpy.types.UILayout:
	box = layout.box()
	row = box.row(align=True)
	col = row.column(align=True)
	col.prop(element, "name")
...
def _draw_component(layout: bpy.types.UILayout, context: bpy.types.Context, component_ref: STF_Component_Ref, context_object: Any, component: Squirrelbite_Avatar_Setup):
	...
	header, body = layout.panel("com.squirrelbite_avatar_setup_grab_toggles", default_closed = True)
	header.label(text="Grab Toggles (" + str(len(component.grab_toggles)) + ")")
	if(body): draw_list(body, "collection", component, "grab_toggles", _blender_property_name, _draw_func_grab_toggle)
