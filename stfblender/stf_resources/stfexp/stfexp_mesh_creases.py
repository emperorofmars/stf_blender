...
class Handler_STF_Mesh_Creases(STF_Handler_Component):
	...
	stf_type = "stfexp.mesh.creases"
	stf_category = STF_Category.COMPONENT
	understood_blender_types = [STFEXP_Mesh_Creases]

	blender_property_name = "stfexp_mesh_creases"
	single = True
	...
	@classmethod
	def export_resource(cls, context: STF_ExportContext, component: STFEXP_Mesh_Creases, context_resource: bpy.types.Mesh | None) -> tuple[dict, str]:
...
class HOOK_STFEXP_Mesh_Creases(STF_ExportComponentHook):
	...
	def hook_can_handle_blender_resource(cls, blender_resource: Any) -> bool:
		...
		if(hasattr(mesh, Handler_STF_Mesh_Creases.blender_property_name) and len(getattr(mesh, Handler_STF_Mesh_Creases.blender_property_name)) > 0): return False
		return True

	@classmethod
	def hook_export_resource(cls, context: STF_ExportContext, blender_resource: bpy.types.Mesh, context_resource: Any):
		add_component(blender_resource, Handler_STF_Mesh_Creases.blender_property_name, str(uuid.uuid4()), Handler_STF_Mesh_Creases.stf_type)


def register():
	setattr(bpy.types.Mesh, Handler_STF_Mesh_Creases.blender_property_name, bpy.props.CollectionProperty(type=STFEXP_Mesh_Creases, options=set()))

def unregister():
	if hasattr(bpy.types.Mesh, Handler_STF_Mesh_Creases.blender_property_name):
		delattr(bpy.types.Mesh, Handler_STF_Mesh_Creases.blender_property_name)
