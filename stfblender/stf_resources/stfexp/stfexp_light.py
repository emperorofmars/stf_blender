...
class STFSetSTFEXPLightIDOperator(bpy.types.Operator, STFSetIDOperatorBase):
	...
	@classmethod
	def poll(cls, context) -> bool: return context.object.stf_instance is not None and context.object.data is not None and isinstance(context.object.data, bpy.types.Light)
...
class Handler_STFEXP_Light(STF_Handler_BlenderNative, STF_Handler_Animation):
	...
	def import_resource(cls, context: STF_ImportContext, json_resource: dict, stf_id: str, context_resource: Any) -> Any | STFReport:
		...
		if(not blender_object or type(blender_object) is not bpy.types.Object):
			return STFReport("Failed to import light", STFReportSeverity.Error, stf_id, cls.stf_type, context_resource)
