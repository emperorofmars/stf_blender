...
class STFEXP_Collider_Plane_LoadJsonOperator(ComponentLoadJsonOperatorBase, bpy.types.Operator):
	...
	def get_property(self, context) -> Any:
		if(not self.blender_bone):
			return getattr(context.object, Handler_STFEXP_Collider_Plane.blender_property_name)
		else:
			return getattr(context.bone, Handler_STFEXP_Collider_Plane.blender_property_name)

	def parse_json(self, context, component: Any, json_resource: dict) -> set[str]:  # pyright: ignore[reportIncompatibleMethodOverride]
		if(json_resource.get("type") != Handler_STFEXP_Collider_Plane.stf_type): raise Exception("Invalid Type")
...
class Handler_STFEXP_Collider_Plane(STF_Handler_BoneComponent, STF_Handler_Animation):
	...
	stf_type = "stfexp.collider.plane"
	stf_category = STF_Category.COMPONENT
	like_types = ["collider.plane", "collider"]
	understood_blender_types = [STFEXP_Collider_Plane]
	blender_property_name = "stfexp_collider_plane"
	single = False
	filter = [bpy.types.Object, bpy.types.Bone]
	pretty_name_template = "Plane Collider"

	@classmethod
	def draw(cls, layout: bpy.types.UILayout, context: bpy.types.Context, component_ref: STF_Component_Ref, context_resource: Any, component: STF_ComponentResourceBase) -> None:
	...
	@classmethod
	def export_resource(cls, context: STF_ExportContext, blender_resource: STFEXP_Collider_Plane, context_resource: Any) -> tuple[dict, str] | STFReport:
	...
	@classmethod
	def update_component_instance(cls, context: bpy.types.Context, component_ref: STF_Component_Ref, context_resource: Any, component: STFEXP_Collider_Plane, component_instance: STFEXP_Collider_Plane):
		component_instance.offset_position = component.offset_position
		component_instance.offset_rotation = component.offset_rotation

	@classmethod
	def export_component_instance(cls, context: STF_ExportContext, component_ref: STF_Component_Ref, component_instance: STFEXP_Collider_Plane, context_resource: Any) -> dict:
		return _serialize_json(component_instance)

	@classmethod
	def import_component_instance(cls, context: STF_ImportContext, json_resource: dict, component_ref: STF_Component_Ref, component_instance: STFEXP_Collider_Plane, context_resource: Any):
		_parse_json(component_instance, json_resource)


def register():
	setattr(bpy.types.Object, Handler_STFEXP_Collider_Plane.blender_property_name, bpy.props.CollectionProperty(type=STFEXP_Collider_Plane))
	setattr(bpy.types.Bone, Handler_STFEXP_Collider_Plane.blender_property_name, bpy.props.CollectionProperty(type=STFEXP_Collider_Plane))

def unregister():
	if hasattr(bpy.types.Object, Handler_STFEXP_Collider_Plane.blender_property_name):
		delattr(bpy.types.Object, Handler_STFEXP_Collider_Plane.blender_property_name)
	if hasattr(bpy.types.Bone, Handler_STFEXP_Collider_Plane.blender_property_name):
		delattr(bpy.types.Bone, Handler_STFEXP_Collider_Plane.blender_property_name)
