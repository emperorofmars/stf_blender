...
class STFEXP_Collider_Capsule(STF_BlenderComponentBase):
...
def _set_component_instance_standin(context: bpy.types.Context, component_ref: STF_Component_Ref, context_object: any, component: STFEXP_Collider_Capsule, standin_component: STFEXP_Collider_Capsule):
...
def _resolve_property_path_to_stf_func(context: STF_ExportContext, application_object: any, application_object_property_index: int, data_path: str) -> tuple[list[str], Callable[[list[float]], list[float]], list[int]]:
	if(match := re.search(r"^stfexp_collider_capsule\[(?P<component_index>[\d]+)\].enabled", data_path)):
		component = application_object.stfexp_collider_capsule[int(match.groupdict()["component_index"])]
...
def _resolve_stf_property_to_blender_func(context: STF_ImportContext, stf_path: list[str], application_object: any) -> tuple[any, int, any, any, list[int], Callable[[list[float]], list[float]]]:
	...
	# let component_index
	for component_index, component in enumerate(application_object.stfexp_collider_capsule):
		if(component.stf_id == blender_object.stf_id):
			break
	match(stf_path[1]):
		case "enabled":
			return None, 0, "OBJECT", "stfexp_collider_capsule[" + str(component_index) + "].enabled", None, None
	return None


class STF_Module_STFEXP_Collider_Capsule(STF_BlenderBoneComponentModule):
	"""Capsule collider"""
	stf_type = _stf_type
	stf_kind = "component"
...
register_stf_modules = [
	STF_Module_STFEXP_Collider_Capsule
]
